"""
Event Consumer for Signal Service with Deduplication Support.
...
This ensures that even if the OPS service dispatcher retries an event
(due to network issues, timeouts, etc.), the Signal Service will:
1. Detect it was already processed
2. Return the cached result instead of reprocessing
3. Maintain exactly-once semantics from a business logic perspective
...
Usage:
    ...
    @app.post("/api/v1/events/market_data_ready")
    async def handle_market_data_ready(event: dict):
        return await consumer.process_event(
            event=event,
            handler=generate_signals_from_market_data,
        )
...
class EventConsumer:
    """
    Event consumer with deduplication support.
    ...
    CRITICAL: This is essential for maintaining data consistency when
    the OPS service dispatcher retries failed event deliveries.
    """
    ...
    async def process_event(
        ...
        if not event_id or not idempotency_key:
            logger.warning(
                "Event missing identifiers, processing without deduplication",
                event=event,
            )
        ...
        if cached is not None:
            ...
            return cached.get("result_data") or {"status": "already_processed", **cached}
        ...
        try:
            ...
            # Determine if handler indicated a skip
...
def with_deduplication(
    ...
    Usage:
        @with_deduplication()
        async def handle_market_data_ready(event: dict) -> dict:
            # Process event
            return {"signals": [...]}
