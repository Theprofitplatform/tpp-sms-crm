#!/usr/bin/env node
const https = require('https');

const SERPBEAR_URL = 'serpbear.theprofitplatform.com.au';
const USERNAME = 'admin';
const PASSWORD = 'coNNRIEIkVm6Ylq21xYlFJu9fIs=';
const DOMAIN = 'instantautotraders.com.au';

console.log('🔧 Updating domain Search Console settings...\n');

// Login
function login() {
  return new Promise((resolve, reject) => {
    const postData = JSON.stringify({ username: USERNAME, password: PASSWORD });
    
    const options = {
      hostname: SERPBEAR_URL,
      port: 443,
      path: '/api/login',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(postData)
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      const cookies = res.headers['set-cookie'];
      
      res.on('data', (chunk) => data += chunk);
      res.on('end', () => {
        if (res.statusCode === 200) {
          console.log('✅ Login successful');
          console.log('   Cookies received:', cookies);
          // Extract just the cookie values without attributes
          const cookieValues = cookies ? cookies.map(c => c.split(';')[0]) : [];
          console.log('   Cookie values:', cookieValues);
          resolve(cookieValues);
        } else {
          reject(new Error(`Login failed: ${res.statusCode}`));
        }
      });
    });

    req.on('error', reject);
    req.write(postData);
    req.end();
  });
}

// Get domain details
function getDomain(cookies) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: SERPBEAR_URL,
      port: 443,
      path: '/api/domains',
      method: 'GET',
      headers: {
        'Cookie': cookies.join('; ')
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      
      res.on('data', (chunk) => data += chunk);
      res.on('end', () => {
        try {
          const result = JSON.parse(data);
          const domain = result.domains?.find(d => d.domain === DOMAIN);
          
          if (domain) {
            console.log('✅ Found domain:', DOMAIN);
            console.log('   Domain ID:', domain.ID);
            resolve({ cookies, domain });
          } else {
            reject(new Error(`Domain ${DOMAIN} not found`));
          }
        } catch (e) {
          reject(e);
        }
      });
    });

    req.on('error', reject);
    req.end();
  });
}

// Update domain with GSC settings
function updateDomain({ cookies, domain }) {
  return new Promise((resolve, reject) => {
    console.log('\n🔍 DEBUG: Cookies being sent:', cookies);
    // Set Search Console to use URL property
    const searchConsoleSettings = {
      property_type: 'url',
      url: `https://${DOMAIN}/`,
      client_email: 'seo-analyst-automation@robotic-goal-456009-r2.iam.gserviceaccount.com',
      private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----'
    };

    const updateData = {
      search_console: searchConsoleSettings
    };

    console.log('\n📝 Updating domain settings:');
    console.log('   Property Type: url');
    console.log('   URL:', searchConsoleSettings.url);

    const postData = JSON.stringify(updateData);
    
    const options = {
      hostname: SERPBEAR_URL,
      port: 443,
      path: `/api/domains?domain=${domain.domain}`,
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(postData),
        'Cookie': cookies.join('; ')
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      
      res.on('data', (chunk) => data += chunk);
      res.on('end', () => {
        if (res.statusCode === 200) {
          console.log('\n✅ Domain settings updated successfully!');
          resolve(cookies);
        } else {
          console.error(`\n❌ Update failed: ${res.statusCode}`);
          console.error(data);
          reject(new Error(`Update failed: ${res.statusCode}`));
        }
      });
    });

    req.on('error', reject);
    req.write(postData);
    req.end();
  });
}

// Main
(async () => {
  try {
    const cookies = await login();
    const domainData = await getDomain(cookies);
    await updateDomain(domainData);
    
    console.log('\n' + '='.repeat(60));
    console.log('🎉 Domain configured to use URL property!');
    console.log('='.repeat(60));
    console.log('\n📋 Next step:');
    console.log('   Run: node trigger-gsc-refresh.cjs');
    console.log('\n✅ GSC data should now appear in SerpBear!');
    console.log('');
    
  } catch (error) {
    console.error('\n❌ Error:', error.message);
    process.exit(1);
  }
})();
