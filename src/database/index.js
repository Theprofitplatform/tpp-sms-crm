...
-- ============================================================
-- OTTO SEO ENHANCEMENT FEATURES
-- ============================================================

-- Pixel deployments (Script-based monitoring)
CREATE TABLE IF NOT EXISTS pixel_deployments (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  client_id TEXT NOT NULL,
  domain TEXT NOT NULL,
  api_key TEXT NOT NULL UNIQUE,
  pixel_code TEXT NOT NULL, -- The actual JavaScript code to embed
  status TEXT DEFAULT 'active', -- 'active', 'inactive', 'suspended'
  deployment_type TEXT DEFAULT 'header', -- 'header', 'body', 'footer'
  pages_tracked INTEGER DEFAULT 0,
  last_ping_at DATETIME, -- Last time pixel sent data
  last_seen_url TEXT, -- Last URL where pixel was active
  features_enabled TEXT, -- JSON array of enabled features ['meta-tracking', 'performance', 'schema']
  metadata TEXT, -- JSON string for additional config
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (client_id) REFERENCES clients(id)
);
CREATE INDEX IF NOT EXISTS idx_pixel_client ON pixel_deployments(client_id);
CREATE INDEX IF NOT EXISTS idx_pixel_api_key ON pixel_deployments(api_key);
CREATE INDEX IF NOT EXISTS idx_pixel_status ON pixel_deployments(status);

-- Pixel page data (Data collected by pixel)
CREATE TABLE IF NOT EXISTS pixel_page_data (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  images_data TEXT, -- JSON array of image objects
  links_data TEXT, -- JSON array of link objects
  schema_found TEXT, -- JSON array of existing schema
  performance_metrics TEXT, -- JSON object with CWV metrics
  issues_detected TEXT, -- JSON array of SEO issues
...
CREATE TABLE IF NOT EXISTS schema_markup (
  ...
  schema_type TEXT NOT NULL, -- 'Article', 'Product', 'LocalBusiness', 'FAQPage', 'HowTo', etc.
  schema_data TEXT NOT NULL, -- JSON-LD markup
  status TEXT DEFAULT 'pending', -- 'pending', 'active', 'inactive', 'removed'
  auto_generated BOOLEAN DEFAULT 1, -- Was this auto-generated by AI?
  applied_at DATETIME,
  removed_at DATETIME,
  validation_status TEXT DEFAULT 'pending', -- 'pending', 'valid', 'invalid'
  validation_errors TEXT, -- JSON array of validation errors
  performance_impact TEXT, -- JSON object tracking before/after metrics
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (client_id) REFERENCES clients(id)
);
CREATE INDEX IF NOT EXISTS idx_schema_client_url ON schema_markup(client_id, page_url);
CREATE INDEX IF NOT EXISTS idx_schema_type ON schema_markup(schema_type);
CREATE INDEX IF NOT EXISTS idx_schema_status ON schema_markup(status);

-- Schema opportunities (AI-detected potential schema markup)
CREATE TABLE IF NOT EXISTS schema_opportunities (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  client_id TEXT NOT NULL,
  page_url TEXT NOT NULL,
  schema_type TEXT NOT NULL,
  confidence_score INTEGER, -- 0-100, how confident AI is about this schema type
  detected_data TEXT, -- JSON object of data extracted from page
  recommendation TEXT, -- AI recommendation for why this schema fits
  status TEXT DEFAULT 'new', -- 'new', 'applied', 'dismissed'
  priority TEXT DEFAULT 'medium', -- 'low', 'medium', 'high'
  estimated_impact TEXT, -- 'low', 'medium', 'high'
  applied_schema_id INTEGER, -- Reference to schema_markup if applied
  dismissed_reason TEXT,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (client_id) REFERENCES clients(id),
  FOREIGN KEY (applied_schema_id) REFERENCES schema_markup(id)
);
CREATE INDEX IF NOT EXISTS idx_schema_opp_client ON schema_opportunities(client_id);
CREATE INDEX IF NOT EXISTS idx_schema_opp_status ON schema_opportunities(status);
CREATE INDEX IF NOT EXISTS idx_schema_opp_priority ON schema_opportunities(priority);

-- Server-side optimizations (For non-CMS platforms)
CREATE TABLE IF NOT EXISTS ssr_optimizations (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  client_id TEXT NOT NULL,
  domain TEXT NOT NULL,
  page_url TEXT NOT NULL,
  optimization_type TEXT NOT NULL, -- 'title', 'meta_description', 'schema', 'hreflang', 'canonical', 'og_tags'
  original_value TEXT, -- Before optimization
  optimized_value TEXT NOT NULL, -- After optimization
  status TEXT DEFAULT 'active', -- 'active', 'inactive', 'testing'
  application_method TEXT DEFAULT 'proxy', -- 'proxy', 'dns', 'cdn', 'direct'
  applied_at DATETIME,
  last_served_at DATETIME, -- Last time this optimization was served
  serve_count INTEGER DEFAULT 0, -- How many times served
  performance_impact TEXT, -- JSON object with metrics
...
CREATE INDEX IF NOT EXISTS idx_ssr_client_url ON ssr_optimizations(client_id, page_url);
CREATE INDEX IF NOT EXISTS idx_ssr_status ON ssr_optimizations(status);
CREATE INDEX IF NOT EXISTS idx_ssr_type ON ssr_optimizations(optimization_type);

-- SSR optimization cache (For fast lookups)
CREATE TABLE IF NOT EXISTS ssr_cache (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  optimization_id INTEGER NOT NULL,
  url_pattern TEXT NOT NULL, -- URL pattern this cache entry matches
  cached_html TEXT, -- Cached optimized HTML fragment
  cache_key TEXT NOT NULL UNIQUE,
  hit_count INTEGER DEFAULT 0,
  last_hit_at DATETIME,
  expires_at DATETIME,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (optimization_id) REFERENCES ssr_optimizations(id)
);
CREATE INDEX IF NOT EXISTS idx_ssr_cache_key ON ssr_cache(cache_key);
CREATE INDEX IF NOT EXISTS idx_ssr_cache_expires ON ssr_cache(expires_at);
...
CREATE TABLE IF NOT EXISTS page_performance (
  ...
  client_id TEXT NOT NULL,
  page_url TEXT NOT NULL,
  source TEXT DEFAULT 'pixel', -- 'pixel', 'lighthouse', 'pagespeed', 'manual'
  ...
  ttfb REAL, -- Time to First Byte (milliseconds)
...
CREATE INDEX IF NOT EXISTS idx_perf_client_url ON page_performance(client_id, page_url);
CREATE INDEX IF NOT EXISTS idx_perf_created ON page_performance(created_at);
`;
